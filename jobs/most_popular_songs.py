...
def main():
    ...
    most_popular_songs = user_playback_df.groupby("artist-name", "track-name").count().alias("play_count").sort(col("count").desc())
    print(most_popular_songs.show(5))

    most_popular_songs2 = user_playback_df.groupby("musicbrainz-artist-id", "musicbrainz-track-id").count().alias("play_count").sort(col("count").desc())
    print(most_popular_songs2.show(5))
